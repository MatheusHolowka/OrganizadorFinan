...
export class AuthService {
  ...
  private async seedDefaultCategories(userId: string) {
    const defaultCategories: Array<{ name: string; type: CategoryType; color: string; icon: string }> = [
      ...
      { name: 'Serviços & Assinaturas', type: CategoryType.EXPENSE, color: '#06B6D4', icon: 'laptop' },
      ...
      { name: 'Educação & Cursos', type: CategoryType.EXPENSE, color: '#10B981', icon: 'graduation-cap' },
      { name: 'Lazer & Entretenimento', type: CategoryType.EXPENSE, color: '#EC4899', icon: 'film' },
      { name: 'Tarifas & Encargos', type: CategoryType.EXPENSE, color: '#64748B', icon: 'receipt' },
      { name: 'Outras Despesas', type: CategoryType.EXPENSE, color: '#94A3B8', icon: 'more-horizontal' },
